...
class TechnewsSpider(scrapy.Spider):
    ...
    def start_requests(self):
        for start_url in self.start_urls:
            n = 2
            yield Request(url=start_url.format(''), callback=self.parse_index)
            while requests.get(url=start_url.format('_0' + str(n))).status_code == 200:
                yield Request(url=start_url.format('_0' + str(n)), callback=self.parse_index)
                n = n + 1
    ...
    def parse_index(self, response):
        ...
        for i in result:
            if i.get('docurl') and re.match('http://.*?.163.com/17', i.get('docurl')):
                yield Request(i.get('docurl'), callback=self.parse_detial)

    def parse_detial(self, response):
        items = News163Item()
        category = parse.urlparse(response.url).netloc
        title = response.xpath('//h1/text()').extract_first()
        post_time, post_source = ''.join(
            response.xpath('//div[@class="post_time_source"]//text()').extract()).strip().replace('\n', '').replace(
            '\u3000', '').split('来源:')
